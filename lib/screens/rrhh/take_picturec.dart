...
class TakePictureCScreen extends StatefulWidget {
  ...
  const TakePictureCScreen({super.key, required this.camera});

  @override
  TakePictureCScreenState createState() => TakePictureCScreenState();
}

class TakePictureCScreenState extends State<TakePictureCScreen> {
  //-----------------------------------------------------------
  //--------------------- Variables ---------------------------
  //-----------------------------------------------------------
        ...
        onPressed: () async {
          try {
            ...
            Response? response = await Navigator.of(context).push(
              MaterialPageRoute(
                builder: (context) => DisplayPicture3Screen(image: image),
              ),
            );
            if (response != null) {
