...
class FlotaSiniestroEditarScreen extends StatefulWidget {
  final VehiculosSiniestro siniestro;
  const FlotaSiniestroEditarScreen({super.key, required this.siniestro});

  @override
  _FlotaSiniestroEditarScreenState createState() =>
      _FlotaSiniestroEditarScreenState();
}

class _FlotaSiniestroEditarScreenState
    extends State<FlotaSiniestroEditarScreen> {
  //---------------------------------------------------------------
  //----------------------- Variables -----------------------------
  //---------------------------------------------------------------
          ...
          SingleChildScrollView(
            child: Column(
              children: <Widget>[
                ...
                _showNumcha(),
                const Divider(height: 5, thickness: 2, color: primaryColor),
                _showCalle(),
                _showNumero(),
                _showCiudad(),
                _showProvincia(),
  ...
  bool validateFields() {
    ...
    if (_detalleDanosTercero.isNotEmpty && _detalleDanosTercero.length > 200) {
      ...
      _detalleDanosTerceroShowError = true;
      _detalleDanosTerceroError =
          'La cantidad máxima de caracteres es de 20. Ha puesto ${_detalleDanosTercero.length}';
    }
    ...
    if (_detalleDanosPropio.isNotEmpty && _detalleDanosPropio.length > 200) {
      ...
      _detalleDanosPropioShowError = true;
      _detalleDanosPropioError =
          'La cantidad máxima de caracteres es de 20. Ha puesto ${_detalleDanosPropio.length}';
    }
  ...
  void _loadFields() async {
    ...
    selectedDate = fechacar;

    int hora = (widget.siniestro.horasiniestro / 3600).floor();
    int minutos = ((widget.siniestro.horasiniestro - hora * 3600) / 60).floor();

    TimeOfDay horaminutos = TimeOfDay(hour: hora, minute: minutos);

    selectedTime = horaminutos;

    _numcha = widget.siniestro.numcha;
    _numchaController.text = widget.siniestro.numcha;

    _calle = widget.siniestro.direccionsiniestro;
    _calleController.text = widget.siniestro.direccionsiniestro;

    _numero = widget.siniestro.altura;
    _numeroController.text = widget.siniestro.altura;

    _ciudad = widget.siniestro.ciudad;
    _ciudadController.text = widget.siniestro.ciudad;

    _provincia = widget.siniestro.provincia;
    _provinciaController.text = widget.siniestro.provincia;

    _numchatercero = widget.siniestro.numchatercero;
    _numchaterceroController.text = widget.siniestro.numchatercero;

    _tercero = widget.siniestro.apellidonombretercero;
    _terceroController.text = widget.siniestro.apellidonombretercero;

    _telefonotercero = widget.siniestro.telefonocontactotercero;
    _telefonoterceroController.text = widget.siniestro.telefonocontactotercero;

    _emailtercero = widget.siniestro.emailtercero;
    _emailterceroController.text = widget.siniestro.emailtercero;

    _companiaseguro = widget.siniestro.companiasegurotercero;
    _companiaseguroController.text = widget.siniestro.companiasegurotercero;
    ...
    _detalleDanosPropio = widget.siniestro.detalledanospropio;
    _detalleDanosPropioController.text = widget.siniestro.detalledanospropio;
