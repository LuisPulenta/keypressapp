...
class ApiHelper {
  ...
  static Future<Response> getSiniestros(String grupo, String causante) async {
    String apiUrl = Preferences.connection;
    var url = Uri.parse(
      '$apiUrl/api/VehiculosSiniestros/GetSiniestros/$grupo/$causante',
    );
  ...
  //---------------------------------------------------------------------------
  static Future<Response> getDetalleCompras(String id) async {
    String apiUrl = Preferences.connection;
    var url = Uri.parse('$apiUrl/api/PEPedidos/GetPEPedidosDetalle/$id');
    var response = await http.post(
      url,
      headers: {
        'content-type': 'application/json',
    ...
    List<PEPedidoDetalle> list = [];
    var decodedJson = jsonDecode(body);
    if (decodedJson != null) {
      for (var item in decodedJson) {
        list.add(PEPedidoDetalle.fromJson(item));
      }
    }
    return Response(isSuccess: true, result: list);
  }
